...
def create_tracklist(
    ...
    output_file = data_dir / "scraped_youtube_tracklist.csv"

    start = time()

    # load main track lists
    feeding_tracklist = pd.read_csv(data_dir / "feeding_tracklist.csv", sep="\t")[
        ["Name", "Artist"]
    ]

    # load the scrapped_tracklist if some tracks are already processed
    if output_file.is_file():
        scrapped_tracklist = pd.read_csv(output_file, index_col=0)
    ...
    scrapped_tracklist.to_csv(output_file)

    # create a txt file with the track indices and youtube ids to be used by the scraper
    with open(data_dir / "scraped_youtube_ids.txt", "w") as youtube_ids_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="data",
        help="directory where the tracklists are located and the scrapped one will be stored",
    )
    parser.add_argument(
        "--scrape_related",
        action="store_true",
